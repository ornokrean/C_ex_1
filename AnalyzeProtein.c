 ...
 */
const char ATOM_LINE_PREFIX[] = "ATOM  ";
...
/**
 * tgus function adds the coordinates of given atom
 ...
 *          using getCoordinate
 */
void addCoordinates(float allAtoms[][NUM_OF_CORD], int numberOfAtoms, char line[LINE_MAX_LEN])
{
...
int addAtom(float allAtoms[][NUM_OF_CORD], int numberOfAtoms, char line[LINE_MAX_LEN])
{
    ...
    if (strstr(line, ATOM_LINE_PREFIX) == line)
    {
        ...
        //add atom cords
        addCoordinates(allAtoms, numberOfAtoms, line);
        numberOfAtoms++;
    }
...
int getFileData(float allAtoms[][NUM_OF_CORD], char *filePath)
{
    ...
    while (fgets(line, LINE_MAX_LEN, file) != NULL && numberOfAtoms < MAX_NUMBER_OF_ATOMS)
    {
...
/**
 * this function prints all the data to the screen
 * @param numberOfAtoms the number of correctly read atoms from the file
 * @param filePath the address of the wanted file
 * @param centerGravity the center gravity of the atoms from the file
 * @param rg the rg of the atoms from the file
 * @param dmax the dmax of the atoms from the file
 */
void
printData(int numberOfAtoms, char *filePath, float centerGravity[NUM_OF_CORD], float rg, float dmax)
{
    // print all the data
...
/**
 * @brief this function prepares the results of the file that were given in filePath
 * @param allAtoms all the atoms that were read from the file
 * @param numberOfAtoms the number of correctly read atoms from the file
 * @param filePath the address of the wanted file
 */
void calcPrintData(float allAtoms[][NUM_OF_CORD], int numberOfAtoms, char *filePath)
{
    ...
    for (int i = 0; i < numberOfAtoms; i++)
    {
        for (int j = i + 1; j < numberOfAtoms; j++)
        {
            ...
            dmax = (tempDistance > dmax) ? tempDistance : dmax;
        }
        rg += powf(getDistance(centerGravity, allAtoms[i]), 2);
    }
    rg = sqrtf(rg / numberOfAtoms);

    printData(numberOfAtoms, filePath, centerGravity, rg, dmax);
}
...
void fileAnalyze(char *filePath)
{
    ...
    calcPrintData(allAtoms, numberOfAtoms, filePath);
